...
export default function PasswordGate({ onUnlock, unlocked }: PasswordGateProps) {
  ...
  return (
    <motion.div
      className={styles.password__wrap}
      variants={rsvpAnim}
      animate={unlocked ? "initial" : "initial"}
      exit="exit"
    >
      ...
      >
        ...
        <button type="submit" disabled={loading} className={styles.pass__but}>
          {loading ? "Validating..." : "Open RSVP Form"}
        </button>
      </form>
    </motion.div>
  );
